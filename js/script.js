...
var getDatefromDayofWeek = function(year, month, pattern_str){
  var weekstr_to_int = { "日" : 0, "月" : 1, "火" : 2, "水" : 3, "木" : 4, "金" : 5, "土" : 6 };
  var day = weekstr_to_int[ pattern_str.trim().substring(0, 1) ];
  var week = Number(pattern_str.trim().substring(1, 2)); 
...
// 個別月によって収集日が変わるごみについて、次回収集日をYYYYMMDD形式で取得
// Input: 月4:1 3 5 7 9 11; 月4:2 4 6 8 10 12;
var getNextCollectionDatefromString = function(format_string){
  var daylist_string = "";
  var date = new Date();

  var date_specification_arr = format_string.split(";");
  for(const m_string of date_specification_arr){
    if(m_string == ""){
      continue;
    }

    var m_string_arr = m_string.split(":");
    
    if(m_string_arr.length != 2){
      return "20000101";
    }

    var collection_pattern_string = m_string_arr[0]; // 月4
    var collection_pattern_string_list = collection_pattern_string.split(" ");
    var month_string_list = m_string_arr[1].split(" "); // 1 3 5 7 9

    for(const month_string of month_string_list){
      // month_string(str)月の収集はcollect_routine_string
      var month_num = Number(month_string);
      
      for(const collection_iter_string of collection_pattern_string_list){
        var next_date_string = getDatefromDayofWeek(date.getFullYear(), month_num, collection_iter_string);
        if(daylist_string != ""){
          daylist_string += " ";
        }
        daylist_string += next_date_string;
        
        var next_date_string_ny = getDatefromDayofWeek(date.getFullYear() + 1, month_num, collection_iter_string);
        if(daylist_string != ""){
          daylist_string += " ";
        }

        daylist_string += next_date_string_ny;
      }
    }
...
var TrashModel = function(_lable, _cell, remarks) {
  ...
  // ;区切りのフォーマットならば、YYYYMMDD形式に変換
  if(_cell.match(/;/)){
    _cell = getNextCollectionDatefromString( _cell );
  }
  ...
  this.getDateLabel = function() {
	...
	return this.getRemark() + "市HPをご確認ください。";
    }
    ...
    for (var i in areaModel.trash) {
	  ...
	  if (trash.mostRecent === undefined) {
	    leftDayText == "不明";
          ...
          if (ableSVG && SVGLabel && description.styles != "") {
